...
Promise.all([
  download(`medivh.js`),
  download(`medivh.min.js`)
]).then(([ devSize, prodSize ]) => {
  // replace installation page version and size
  fs.writeFileSync(
    installPath,
    installation
      .replace(/medivh_version: .*/, 'medivh_version: ' + version)
      .replace(/gz_size:.*/g, `gz_size: "${prodSize}"`)
      .replace(/\/vue@[\d\.]+\//g, `/medivh@${version}/`)
  )
  console.log(`\nSuccessfully updated Medivh version and gzip file size.\n`)
}).catch(err => {
  console.error(err)
  process.exit(1)
})
...
function download (file) {
  return new Promise((resolve, reject) => {
    request({
      url: `http://unpkg.com/medivh@${version}/dist/${file}`,
      encoding: null
    }, (err, res, body) => {
      if (err) {
        return reject(err)
